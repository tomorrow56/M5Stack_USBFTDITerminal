 ...
 */
#if !defined(__CDCFTDIMOD_H__)
#define __CDCFTDIMOD_H__
...
#define FTDI_PID                        0x6001  // FTDI PID FT232x
#define FTDI_PID2                       0x6015  // FTDI PID FT231x

#define FT232AM                         0x0200
#define FT232BM                         0x0400
#define FT2232                          0x0500
#define FT232R                          0x0600
#define FT231X                          0x1000
...
class FTDIMODAsyncOper {
public:

        virtual uint8_t OnInit(FTDIMOD *pftdi __attribute__((unused))) {
                return 0;
        };

        virtual uint8_t OnRelease(FTDIMOD *pftdi __attribute__((unused))) {
                return 0;
        };
};
...
class FTDIMOD : public USBDeviceConfig, public UsbConfigXtracter {
        static const uint8_t epDataInIndex; // DataIn endpoint index
        static const uint8_t epDataOutIndex; // DataOUT endpoint index
        static const uint8_t epInterruptInIndex; // InterruptIN  endpoint index

        FTDIMODAsyncOper *pAsync;
        USB *pUsb;
        uint8_t bAddress;
...
public:
        FTDIMOD(USB *pusb, FTDIMODAsyncOper *pasync, uint16_t idProduct = FTDI_PID);
